...
if [[ "$my_2023_macbook" =~ "$(uname -n)" && -z "$1" ]] || [[ $1 == "mine" ]]; then
    ...
    alias rosa='ssh -L 8180:127.0.0.1:8180 rosa | tee /dev/tty | python3 ~/.ssh/run_webbrowser.py'
    alias rosal='ssh -L 8180:localhost:8180 rosa | tee /dev/tty | python3 ~/.ssh/run_webbrowser.py'
	anvi-activate-dev () {
